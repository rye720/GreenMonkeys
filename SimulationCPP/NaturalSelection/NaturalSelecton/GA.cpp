...
* Function: Recursively combines two top ranking Animals (via fitness) until there is 1 or less animals left in
...
void GA::combination(std::vector<Animal> &pop, int animalNum, int currentGen){
	...
	GAUtils gu = GAUtils();
	if (pop.size() > 1) {
	  // Why not just initialize them randomly in a constructor for Animal?
		sex = gu.randSex();
		//  I would use more descriptive variable names. d? m? what are these?
		Animal m, d;
		int dRand, mRand;

		/*
		 * This could use re-working. A lot of the initialization code can be
		 * moved into the Animal class so that cleanup is worth doing.
		 *
		 * Once you do that, you can do something like
		 * sort(vector_of_animals, by_fitness_function)
		 * if len(vector_of_animals) > 1:
		 *   vector_of_animals.append_to_end(
		 *     vector_of_animals[0].mateWith(vector_of_animals[1]))
		 *   vector_of_animals.remove[0]
		 *   vector_of_animals.remove[1]
		 *   combination(vector_of_animals)
		 * else:
		 *   return vector_of_animals[0]
		 *
...
* Output: Modified Vector of Animals
* Function: Removes 1/4 of the population; lowest fitness is removed
* Notes: None
*/
...
* Function: Calls the function to sort the Vector
